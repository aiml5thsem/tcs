        ...
        try:
            ...
            if sitemap_locs:
                ...
                for sitemap_loc in sitemap_locs:
                    ...
                    if sub_sitemap_url and sub_sitemap_url.startswith('http'):
                ...
                for url_loc in url_locs:
                    url = url_loc.text
                    if url and url.startswith('http'):  # Validate URL format
...
class URLSorter:
    """Sort URLs in logical documentation order."""
    
    # Common documentation section patterns in priority order
    SECTION_PRIORITY = [
        'index', 'home', 'welcome', 'intro', 'overview',
        'getting-started', 'quickstart', 'tutorial', 'guide',
        'installation', 'install', 'setup',
        'basics', 'fundamentals', 'core', 'concepts',
        'usage', 'using', 'working',
        'features', 'capabilities',
        'api', 'reference', 'docs', 'documentation',
        'advanced', 'deep-dive',
        'deployment', 'production',
        'troubleshooting', 'faq', 'help',
        'changelog', 'updates', 'releases', 'news',
        'contributing', 'development', 'internals',
    ]
    
    @staticmethod
    def get_url_priority(url: str) -> tuple:
        """
        Return priority tuple for sorting URLs.
        Lower numbers = higher priority (sorted first).
        Returns: (section_priority, path_depth, alphabetical)
        """
        parsed = urlparse(url)
        ...
        section_priority = 999  # default low priority
        for i, section in enumerate(URLSorter.SECTION_PRIORITY):
            if section in path:
                section_priority = i
                break
        
        # Use path for alphabetical sorting
        return (section_priority, depth, path)
    
    @staticmethod
    def sort_urls(urls: Set[str]) -> List[str]:
        """Sort URLs in logical documentation order."""
        return sorted(urls, key=URLSorter.get_url_priority)
...
class AuthenticatedScraper:
    ...
    async def crawl_auto(self, start_urls: Set[str]):
        ...
        # Sort URLs in logical documentation order
        sorted_start_urls = URLSorter.sort_urls(start_urls)
        
        # Show first 10 URLs in crawl order
        if len(sorted_start_urls) > 1:
            print("📋 Crawl order (first 10):")
            for i, url in enumerate(sorted_start_urls[:10], 1):
                print(f"  {i}. {url}")
            if len(sorted_start_urls) > 10:
                print(f"  ... and {len(sorted_start_urls) - 10} more\n")
            print()
        
        # Use breadth-first search for proper ordering
        queue = deque([(url, 0, None) for url in sorted_start_urls])
        ...
        while queue and len(self.visited_urls) < self.config.max_pages:
            ...
            if content:
                ...
                # Sort child links before adding to queue
                sorted_children = URLSorter.sort_urls(child_links)
                
                for child_url in sorted_children:
    ...
    async def crawl_hybrid(self, discovered_urls: Set[str]):
        ...
        print("\n" + "="*70)
        print("OPTIONS:")
        print("  1. Press ENTER to start crawling all discovered URLs")
        print("  2. Type 'add' to manually add URLs")
        print("  3. Type 'tabs' to open tabs and add them interactively")
        print("  4. Type 'skip' to review and skip specific URLs")
        print("="*70 + "\n")
        
        choice = input("Your choice (ENTER/add/tabs/skip): ").strip().lower()
        
        if choice == 'add':
            # User wants to add URLs manually
            print("\nEnter additional URLs (one per line, empty line to finish):")
            while True:
            ...
            # Interactive tab-based addition (like manual mode)
            print("\n" + "="*70)
            print("📑 TAB-BASED URL ADDITION")
            print("="*70)
            print("\nINSTRUCTIONS:")
            print("  • Open pages in new tabs that you want to add")
            print("  • Press ENTER to detect and add all open tabs")
            print("  • Type 'done' when finished adding tabs\n")
            
            tabs_added = 0
            while True:
                user_input = input("⏳ Press ENTER to detect tabs (or 'done' to finish): ").strip().lower()
                ...
                for page_obj in self.context.pages:
                    try:
                        ...
                        if filtered and filtered not in discovered_urls:
                            discovered_urls.add(filtered)
                            title = await page_obj.title()
                            print(f"  ✓ Added from tab: {title} ({filtered[:60]})")
                            tabs_added += 1
                        elif filtered in discovered_urls:
                            print(f"  ⊘ Already in list: {current_url[:60]}")
                    
                    except Exception as e:
                        print(f"  [!] Error reading tab: {str(e)[:60]}")
                
                if tabs_added > 0:
                    print(f"\n  📊 Total URLs now: {len(discovered_urls)}\n")
            
            print(f"\n✓ Added {tabs_added} URLs from tabs")
            print(f"✓ Total URLs to crawl: {len(discovered_urls)}\n")
        
        elif choice == 'skip':
            # Let user remove URLs
            print("\n" + "="*70)
            print("🗑️  SKIP SPECIFIC URLs")
            print("="*70)
            print("\nEnter URL patterns to skip (one per line, empty line to finish):")
            print("Examples: '/api/', 'logout', 'download'\n")
            
            skip_patterns = []
            while True:
                pattern = input("  Skip pattern: ").strip()
                if not pattern:
                    break
                skip_patterns.append(pattern.lower())
                print(f"    ✓ Will skip URLs containing: {pattern}")
            ...
            if skip_patterns:
                ...
                skipped = original_count - len(filtered_urls)
                discovered_urls = filtered_urls
                print(f"\n✓ Skipped {skipped} URLs")
                print(f"✓ Remaining URLs to crawl: {len(discovered_urls)}\n")
        
        else:
            # Default: just check for any tabs user might have opened
            for page_obj in self.context.pages:
                try:
                    current_url = page_obj.url
                    filtered = self.url_filter.filter_url(current_url)
                    if filtered and filtered not in discovered_urls:
                        discovered_urls.add(filtered)
                        print(f"  ✓ Added from open tab: {current_url[:70]}")
                except:
                    pass
