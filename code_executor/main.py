...
class MCPClientPool:
    ...
    async def call_tool(self, server_name: str, tool_name: str, arguments: Dict) -> Any:
        """Call a tool on a server with robust result unwrapping."""
        try:
            ...
            # Robust result unwrapping with multiple fallback strategies
            
            # Try result.data first (FastMCP Output wrappers e.g., web_searchOutput)
            if hasattr(result, 'data') and result.data is not None:
                data = result.data
                data_type = type(data).__name__
                
                # Handle FastMCP Output wrappers (e.g., web_searchOutput)
                if data_type.endswith('Output') and hasattr(data, 'result'):
                    inner = data.result
                    # Check if inner is empty Result()
                    if type(inner).__name__ == 'Result' and hasattr(inner, '__dict__') and not inner.__dict__:
                        # Empty Result, use structured_content instead
                        pass
                    else:
                        # Has data, return it
                        if hasattr(inner, 'model_dump'):
                            ...
                            return inner
                        elif isinstance(inner, (int, float, str, bool, list, dict)):
                            return inner
                        else:
                            return inner
                
                # For non-Output types, return as-is if primitive
            ...
            if hasattr(result, 'structured_content') and result.structured_content:
                # Extract from 'result' key if present
