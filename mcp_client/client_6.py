...
Complete Features:
✅ YAML/JSON configuration with intelligent merging
✅ Environment variable support (.env + ${VAR} syntax)
✅ Multiple LLM profiles with custom system prompts
✅ Intelligent image management (prevents token explosion)
...
class LLMProfile:
    ...
    def get_fallback_models(self) -> List[str]:
        ...
        return [f"{fb.get('provider', 'openai')}/{fb.get('model', '')}" for fb in self.fallbacks]
...
class MCPUniversalClient:
    """Universal MCP Client with YAML/JSON configuration and intelligent image management"""
    ...
    def __init__(self, 
        ...
        # Check for legacy mcp_settings.json
        if not self.mcp_config.get("servers"):
            legacy_config = ConfigLoader.load_config("mcp_settings", ['.json'])
        ...
        # Check for legacy llmProfiles
        if not self.llm_config.get("profiles"):
            legacy_config = ConfigLoader.load_config("mcp_settings", ['.json'])
            if legacy_config.get("llmProfiles"):
                self.llm_config["profiles"] = legacy_config["llmProfiles"]
                if legacy_config.get("defaultProfile"):
                    self.llm_config["defaultProfile"] = legacy_config["defaultProfile"]
        
        console.print("[green]✓ Configuration loaded[/green]\n")
        
        # Load LLM profiles
        self.profiles = self._load_llm_profiles()
        self.current_profile_name = self.llm_config.get("defaultProfile", list(self.profiles.keys())[0] if self.profiles else "default")
    ...
    async def connect_servers(self):
        ...
        console.print(f"[cyan]Connecting to {len(servers)} MCP server(s)...[/cyan]\n")
    ...
    def _count_images_in_messages(self, messages: List[Dict]) -> int:
        """Count total number of images in message history"""
        count = 0
        for msg in messages:
            content = msg.get("content")
    ...
    def _should_keep_image_in_history(self, message: Dict, current_index: int, total_messages: int) -> bool:
        """Determine if an image should be kept in conversation history"""
        messages_from_end = total_messages - current_index
        return messages_from_end <= 6  # Keep last 3 exchanges
    
    def _cleanup_old_images_from_messages(self, messages: List[Dict]) -> List[Dict]:
        """Remove images from old messages to prevent token explosion"""
        cleaned_messages = []
        total = len(messages)
        ...
        for idx, msg in enumerate(messages):
            ...
            # System/tool/assistant messages pass through unchanged
            if role in ["system", "tool", "assistant"]:
                cleaned_messages.append(msg)
                continue
            
            # For user messages, check if we should keep images
            if role == "user" and isinstance(content, list):
                if self._should_keep_image_in_history(msg, idx, total):
                    cleaned_messages.append(msg)
                else:
                    ...
                    for item in content:
                        if isinstance(item, dict):
                            ...
                            elif item.get("type") in ["image_url", "audio_url", "file"]:
                    ...
                    if text_only_content:
                        cleaned_messages.append({
                            "role": role,
                            "content": text_only_content if len(text_only_content) > 1 else text_only_content[0].get("text", "")
                        })
            else:
                cleaned_messages.append(msg)
    ...
    async def chat(self,
                   ...
                   keep_all_images: bool = False) -> str:
        """
        Main chat function with intelligent image management.
        
        Args:
            messages: Conversation messages
            profile_override: Temporarily use different profile
            max_iterations: Override max iterations
            keep_all_images: If True, keeps all images (may hit token limits)
        """
        ...
        processed_messages = self._prepare_messages_for_llm(messages)
        
        # Clean up old images if not explicitly keeping all
        if not keep_all_images:
            image_count_before = self._count_images_in_messages(processed_messages)
            
            if image_count_before > 3:
                processed_messages = self._cleanup_old_images_from_messages(processed_messages)
                image_count_after = self._count_images_in_messages(processed_messages)
                
                if image_count_before != image_count_after:
                    console.print(f"[dim]  Optimized: {image_count_before} → {image_count_after} images[/dim]")
        
        prepared_messages.extend(processed_messages)
        ...
        while iteration < max_iter:
            ...
            try:
                ...
                if hasattr(assistant_message, 'tool_calls') and assistant_message.tool_calls:
                    ...
                    # Clean up images before next iteration
                    if not keep_all_images:
                        prepared_messages = self._cleanup_old_images_from_messages(prepared_messages)
                ...
                if "rate" in str(e).lower() or "quota" in str(e).lower():
                    console.print("[yellow]  Tip: Rate limit hit. Try /profile to switch models[/yellow]")
    ...
    async def interactive_mode(self):
        ...
        console.print(Panel.fit(
            ...
            f"[dim]FastMCP 2.0 + Intelligent Image Management[/dim]\n\n"
            f"[green]Profile:[/green] {self.current_profile.name if self.current_profile else 'None'}\n"
            f"[blue]Model:[/blue] {self.current_profile.get_model_name() if self.current_profile else 'None'}\n"
    ...
    async def _handle_command(self, command: str, pending_attachments: List, conversation: List) -> tuple[bool, List, List]:
        ...
        if cmd == "/profiles":
            ...
            if not self.all_tools:
                ...
                table = Table(title="Available Tools", box=ROUNDED)
            ...
            image_count = self._count_images_in_messages(conversation)
            
            console.print(f"[cyan]Conversation Statistics:[/cyan]")
            console.print(f"  Messages: {total_messages}")
            console.print(f"  Images: {image_count}")
            console.print(f"  Profile: {self.current_profile_name}")
