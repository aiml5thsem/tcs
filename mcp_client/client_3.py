...
class MCPUniversalClient:
    ...
    def __init__(self, config_path: str = "mcp_settings.json",system_prompt_file: Optional[str] = None):
        ...
        self.default_system_prompt = """You are a helpful AI assistant with access to MCP tools and resources."""
        if system_prompt_file:
            try:
                with open(system_prompt_file, 'r', encoding='utf-8') as f:
                    self.default_system_prompt = f.read()
                console.print(f"[green]✓ Loaded system prompt from {system_prompt_file}[/green]")
            except Exception as e:
                console.print(f"[yellow]⚠ Could not load system prompt: {e}[/yellow]")
